...
export async function startVideoKYC(from) {
  ...
  await sendText(from,
    ...
    `1️⃣  Hold your ID card next to your face\n` +
    `2️⃣  State your name and address clearly\n` +
    `3️⃣  Confirm your loan amount (₹25,000)\n\n` +
    `⚠️ The officer will NEVER ask for your OTP or PIN.`
  );
  ...
  updateSessionData(from, {
    kyc: { link, roomId, generatedAt: new Date().toISOString(),
           expiryTime: expiryTime.toISOString(), completed: false },
  });
  ...
  await sendButtons(
    ...
    `⏱️ Link valid for *${EXPIRY_MIN} minutes* (expires at ${formatTime(expiryTime)}).`,
    [
      { id: 'kyc_ready', title: '📹 I am ready to join' },
      { id: 'kyc_help',  title: '❓ I need help'         },
    ],
    '🎥 Video KYC — Final Step',
    'Officers available: 9 AM – 6 PM'
  );
}
...
export async function handleKYCReady(from) {
  ...
  if (isExpired(data.kyc?.expiryTime)) {
    await sendText(from, `⚠️ Your link has expired. Generating a new one...`);
    await pause(600);
  ...
  await sendButtons(
    ...
    `Checklist:\n☑️ ID card in hand\n☑️ Good lighting\n☑️ Camera & mic allowed in browser\n\n` +
...
export async function handleKYCDone(from) {
  ...
  updateSessionData(from, {
    kyc: { ...getSession(from).data.kyc, completed: true, completedAt: new Date().toISOString() },
  });
  ...
  await sendText(from,
    ...
    `_Please do not close this chat._`
  );
...
export async function handleKYCHelp(from) {
  await sendButtons(
    from,
    `❓ *Video KYC Help*\n\n` +
    `*Link not opening?* → Open in Chrome or Firefox\n` +
    `*Camera not working?* → Allow camera permission when asked\n` +
    `*Call disconnected?* → Get a fresh link and rejoin\n` +
    `*Officer not available?* → Available 9 AM–6 PM on working days\n\n` +
...
// ── Text reminder during KYC states ───────────────────────────────────────
...
async function sendApproval(from) {
  ...
  const disburseDate = getNextWorkingDay();

  updateSessionData(from, {
    approval: { approved: true, loanRef,
                approvedAt: new Date().toISOString(),
                disburseDate: disburseDate.toISOString() },
  });
  ...
  await sendText(from,
    ...
    `📅 Disbursement:  ${formatDate(disburseDate)}\n` +
    `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
